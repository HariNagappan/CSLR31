...
system_clock::time_point parseTime(const string &timeStr)
{
...
namespace Utils
{
    ...
    */
        ...
        ***********************************************************************************************/
        ...
        static vector<LatLon> generatePath(const vector<LatLon> &waypoints, const Params &params)
        {
            ...
            auto choose_spacing_at = [&](const LatLon &p) -> double
            {
                ...
                if (dmin <= near_radius_m)
                    return max(min_spacing_m, near_spacing_m);

                if (dmin <= mid_radius_m)
                    return max(min_spacing_m, mid_spacing_m);
            ...
            for (size_t i = 0; i + 1 < waypoints.size(); ++i)
            {
                ...
                if (Geo::distance_m(out.back(), B) > 0.5)
                    out.push_back(B);
            ...
            if (out.empty() || Geo::distance_m(out.back(), lastWp) > 0.5)
                out.push_back(lastWp);

            // Optionally: remove near-duplicate consecutive points (safety cleanup)
            vector<LatLon> cleaned;
            for (const auto &p : out)
            {
                if (cleaned.empty() || Geo::distance_m(cleaned.back(), p) > 0.5)
                    cleaned.push_back(p);
            }
    ...
    public:
        ...
        bool canCollide()
        {
            auto flight_start = parseTime(flightData.depart_time);
            auto bird_start = parseTime(birdData.t_start);
            auto bird_end = parseTime(birdData.t_end);
            auto flight_end = flight_start; // TODO check flight end time
            for (int i = 0; i < flightData.waypoints.size() - 1; i++)
            {
                auto wp1 = flightData.waypoints[i];
                auto wp2 = flightData.waypoints[i + 1];
                double dist = Utils::Geo::distance_m(wp1.pos, wp2.pos);
                double time_needed_s = (dist / (speed * 0.514444));
                flight_end += seconds((int)time_needed_s);
            }
            if (bird_start > flight_end || bird_end < flight_start)
            {
                return false;
            }
            for (int i = 0; i < flightData.waypoints.size(); i++)
            {
                for (int j = 0; j < birdData.corridor.size(); j++)
                {
                    double dist = Utils::Geo::distance_m(flightData.waypoints[i].pos, birdData.corridor[j].pos);
                    if (dist <= birdData.lateral_buffer_km * 1000)
                    {
                        return true;
                    }
                }
        ...
        vector<FlightWaypoint> getAlternativeFlightPath()
        {
            if (!canCollide())
            {
                return flightData.waypoints;
            }
            vector<FlightWaypoint> alternativePath;
            for (int i = 0; i < flightData.waypoints.size(); i++)
            {
                bool safe = true;
                LatLon birdPos;
                for (int j = 0; j < birdData.corridor.size(); j++)
                {
                    ...
                    {
                        safe = false;
                        birdPos = birdData.corridor[j].pos;
                        break;
                    }
                }
                if (safe)
                {
                    alternativePath.push_back(flightData.waypoints[i]);
                }
                else
                {
                    // Shift waypoint 1 km away from bird corridor point
                    double bearing = Utils::Geo::bearing_deg(birdPos, flightData.waypoints[i].pos);
                    LatLon newPos1 = Utils::Geo::destinationPoint(flightData.waypoints[i].pos, bearing, birdData.lateral_buffer_km);
                    LatLon newPos2 = Utils::Geo::destinationPoint(flightData.waypoints[i].pos, -bearing, birdData.lateral_buffer_km);
                    double dist1 = Utils::Geo::distance_m(newPos1, birdPos);
                    double dist2 = Utils::Geo::distance_m(newPos2, birdPos);
                    if (dist1 <= birdData.lateral_buffer_km)
                    {
                        FlightWaypoint newWp = flightData.waypoints[i];
                        newWp.pos = newPos1;
                        alternativePath.push_back(newWp);
                        continue;
                    }
                    if (dist2 <= birdData.lateral_buffer_km)
                    {
                        FlightWaypoint newWp = flightData.waypoints[i];
                        newWp.pos = newPos2;
                        alternativePath.push_back(newWp);
