...
public:
    ...
    ***********************************************************************************************/
    ...
    ) 
    {
        ...
        for (size_t i = 0; i < samples.size(); ++i) 
        {
            if (samples[i].rh_percent < driestRH) 
            { driestRH = samples[i].rh_percent; driestIdx = (int)i; }
        }
...
int main() 
{
    ...
    auto res1 = WeatherFlightSafetyEnhanced::recommend_with_location(currentLoc, curAltM, curTempC, curRH, wind_ms, samples, 2.0, 5000.0, 2000.0, 10.0);
    ...
    auto res2 = WeatherFlightSafetyEnhanced::recommend_estimatedT(curr2, alt2, rh2, wind2, samples2, 30.0 ); //sea-level temp (30.0 °C)
